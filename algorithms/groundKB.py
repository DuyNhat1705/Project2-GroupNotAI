...
class KnowledgeBase:
    ...
    # ── FC-compatible interface ───────────────────────────────────────────────

    def _init_domains(self):
        """Initialise per-cell value domains from the current puzzle grid."""
        self.domains = {}
        for i in range(1, self.size + 1):
            for j in range(1, self.size + 1):
                fixed_val = self.riddle.grid[i - 1][j - 1]
                if fixed_val != 0:
                    self.domains[(i, j)] = {fixed_val}
                else:
                    self.domains[(i, j)] = set(self.domain)

    def init_horn_rules(self):
        """Placeholder for Horn clause rule initialisation."""
        self.horn_rules = []

    def add_val(self, i, j, v):
        """Assert Val(i, j, v) into observed facts (idempotent)."""
        atom = Val(i, j, v)
        if atom not in self.obs_facts:
            self.obs_facts.append(atom)

    def has_val(self, i, j, v):
        """Return True if Val(i, j, v) is already a known fact."""
        return Val(i, j, v) in self.obs_facts

    def is_contradiction(self):
        """Return True if any cell domain is empty."""
        return any(len(d) == 0 for d in self.domains.values())

    def is_solved(self):
        """Return True if every cell has exactly one value in its domain."""
        return all(len(d) == 1 for d in self.domains.values())

    def clone(self):
        """Return a deep copy suitable for backtracking."""
        new_kb = object.__new__(KnowledgeBase)
        new_kb.riddle       = self.riddle       # immutable — shared
        new_kb.size         = self.size
        new_kb.domain       = self.domain       # immutable — shared
        new_kb.axioms       = self.axioms       # immutable — shared
        new_kb.ground_rules = self.ground_rules # immutable — shared
        new_kb.horn_rules   = self.horn_rules   # immutable — shared
        new_kb.obs_facts    = list(self.obs_facts)
        new_kb.domains      = {k: set(v) for k, v in self.domains.items()}
        return new_kb

    def get_result_grid(self):
        """Return a 2-D list of solved values (0 for unsolved cells)."""
        grid = []
        for i in range(1, self.size + 1):
            row = []
            for j in range(1, self.size + 1):
                d = self.domains[(i, j)]
                row.append(next(iter(d)) if len(d) == 1 else 0)
            grid.append(row)
        return grid
